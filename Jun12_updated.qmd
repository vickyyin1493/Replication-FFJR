...
```
After calculating the abnormal returns according to FFJR's approach, we can now replicate a critical aspect of the FFJR paper, which is essentially Figure 2b. However, a difference between the graph below and FFJR's Figure 2b is that this figure outlines the relationship between the cumulative average residuals of all splits but primarily focuses on the announcement date instead of the effective date. Additionally, the x-axis is in terms of "days" in our replication which is more specific than FFJR's month approach due to the lack of computational abilities at the time. 
