...
Given the above, we therefore discussed alternative theories during our seminar. Particularly, we believe that the following theories that constitute extensions of Theories A and B are applicable in FFJR and our results obtained below. 

Theory A': Firms like to keep their stock price within certain bounds. When the stock price rises above a certain threshold, a firm may initiate a split, announcing it several weeks in advance of its effective date. Firms do not use splits to signal private information about firm prospects. Pre-split stock prices may be driven by information that suggests an imminent increase in dividends. 

**Theory B'':** Firms use splits to signal private information about future dividend increases, which in turn signal private information about firm prospects. A firm will announce a split several weeks in advance of its effective date. 

These theories and their applicability to our replication and FFJR will be explained in the main replication and dividend analyses below.
...
split_returns_reg <-
  ...
  distinct() 
```
We now estimate regressions and calculate the abnormal returns in a similar approach to FFJR. In particular, a market model is esitmated by regressing the log of gross returns of each stock on the log of gross returns of a market index before taking the residual as shown in FFJR.
```{r}
#| include: false
abnormal_returns <-
  split_returns_reg %>%
...
Regarding the theory alignment, it can be identified that FFJR primarily argues for Theory B within the paper where it is mentioned that a proposed split may be utilised to be a "source of information" to reduce uncertainty (page 2). Again, this theory as revealed in the introduction, aligns more closely with the information signalling hypothesis that firms will reveal information regarding the firm’s positive prospects to the market. However, the figure obtained below aligns more closely with Theory A, which denotes the liquidity hypothesis where management utilises stock splits to enhance their stock’s liquidity. This can be observed by the increase in stock returns prior to the stock split announcement, which again infers that firms will implement a stock split when security prices are high, indicated by the positive or rising returns. Hence, this shows that the liquidity hypothesis is both applicable to the market in 1926-1960 and the current market as well. Furthermore, it can still be deemed that Theory A is the theory that is consistent with the graph pattern observed in comparison to the information signalling hypothesis, denoted by Theory B. 
```{r}
table2_data %>%
  group_by(day_gap) %>%
...
In order to distinguish theory A (liquidity hypothesis) and theory B (information signalling hypothesis), the addition of an analysis concerning the relationship between stock splits and dividends would be significant to analyse. 

From the introduction, Theory B suggests that firms implement stock splits to signal dividend announcements which in turn may also signal future prospects. There are two components in this theory, one is that stock splits signals dividends and another one is that dividends may signal future prospects.
...
Assuming theory B holds, then the market should react positively when splits are first announced and will not subsequently react for firms that announced dividend increases but react negatively for firms that announced dividend decreases. This is due to the (assumed) market's expectation that firms announced stock splits will announce dividend increases (theory B) as they were using stock splits as a signalling tool, and when the market's expectation are not being met, it would react through negative returns.

As firms could pay dividends multiple times per day (although it might be highly unlikely), we followed the approach in the textbook and calculated daily dividend amounts. We then selected NYSE stocks for our purpose of the analysis. 

Following FFJR, for a given day t, we calculated total dividends paid from day t-364 to day t (div_trailing), and also from day t+1 to day t+365 (div_forward). This is to calculate the 'dividend change ratio' as defined in FFJR. We followed the approach in the textbook to calculate total dividend per equivalent unsplit share using cfacshr and treat no dividend as divamt = 0. 

We encountered some problems in specifying meaningful cases. We were deciding whether we should follow a similar approach in the textbook and exclude cases where a stock has listed within the last twelve months or delists in the subsequent months. When we were analysing the distributions of days_trailing and days_forward which are the numbers of days a stock is listed from t-364 to t and from t+1 to t+365, we found that a lot more firms have 365 days before and after compared to any other numbers of days. While it could be problematic to restrict exactly 365 days (as suggested by Ian), it might be the better number to use compared to others. However, using this approach gives us significantly less data points compared to month-based restrictions, while leaving out this restriction may yield us less-meaningful dividend change ratio. We decided to turn to alternative approach as suggested by Ian and aggregated the data by month before calculating.  
...
In the next chunk, we aggregated the data by month, excluded less-meaningful cases and then proceeded by using daily data without filtering to calculate daily dividend amounts.
...
We are interested in the dividends paid by NYSE firms that announced splits. We used full-join here rather than left_join in the original replication. This is because using left_join would give us substantially less data points for split_firm_dividends, specifically, we will be left with only 1 split_firm_dividends while full_join gave us all data points. 
However, we will be left with a lot of NA values for dclrdt (announcement dates) using full_join, luckily, the day column captures the same information and is not lost in the joining process. We then changed the following code to use day instead of dclrdt.
...
'Dividend increases' (up_div) here is defined as when the split firm's dividend change ratio is greater than the market dividend change ratio in a given day. 
...
As we named both date and dclrdt as day previously, we wanted to specify a value to use when plotting the dividend data, here, we used date to produce an analogue of figure 3c from FFJR.
